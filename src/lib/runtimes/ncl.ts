...
export async function executeNCL(code: string, tmpDir: string, stdin?: string): Promise<NCLResult> {
  const startTime = Date.now();
  const filePath = path.join(tmpDir, "script.ncl");
  await fs.writeFile(filePath, code, "utf-8");

  // 1. Check if real system NCL binary is available
  try {
    const isNCLInstalled = await new Promise<boolean>((resolve) => {
      execFile("which", ["ncl"], (err, stdout) => {
        resolve(!err && stdout.trim().length > 0);
      });
    });

    if (isNCLInstalled) {
      const realExec = await new Promise<{ stdout: string; stderr: string; exitCode: number }>((resolve) => {
        execFile("ncl", ["-Q", filePath], { cwd: tmpDir, timeout: 15000 }, (error, stdout, stderr) => {
          resolve({
  ...
  try {
    const stdoutLogs: string[] = ["NCAR Command Language Version 6.6.2 (BOSSgt Studio Sandbox)"];
    ...
    // Save SVG file
    const svgPath = path.join(tmpDir, `${filenameBase}.svg`);
    await fs.writeFile(svgPath, svgGraphic, "utf-8");
...
async function scanForGraphicsFiles(dir: string): Promise<{ name: string; url: string; type: string }[]> {
